...
public class TextTracksService implements TextTracksServiceInterface {
    ...
    @Override
    public String getSubtitle(String id, String langTag) throws ExtractionException, IOException, SubtitleNotFoundException {
        ...
        if (subtitle == null) {
            ...
            for (SubtitlesStream st : info.getSubtitles()) {
                if (st.isAutoGenerated()) continue;

                if (Objects.equals(st.getLanguageTag(), langTag)) {
                    String url = st.getContent().replace("&fmt=ttml", "&fmt=vtt");

                    RestTemplate restTemplate = new RestTemplate();
                    subtitle = restTemplate.getForObject(url, String.class);
                }
            }
        }
